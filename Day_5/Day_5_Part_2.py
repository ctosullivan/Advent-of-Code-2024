...
def fix_update(update_array, rules_dict):
    # Function to reorder an update array so that it becomes valid
    # Returns the reordered array
    while not check_update(update_array, rules_dict):
        for page in update_array:
            if page in rules_dict.keys():
                order_array = rules_dict[page]
                for rule in order_array:
                    if rule in update_array:
                        if update_array.index(page) > update_array.index(rule):
                            update_array[update_array.index(page)] , update_array[update_array.index(rule)] = update_array[update_array.index(rule)], update_array[update_array.index(page)]
    return(update_array)

result = 0
for array in updates_array:
    if not check_update(array, rules_dict):
        array = fix_update(array, rules_dict)
